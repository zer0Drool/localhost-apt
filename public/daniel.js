...
function heartClick() { // the user has created a heart
    ...
    socket.emit('newHeart', {colour: user.colour, userPosition: 'daniel'}); // sends heart to the server so it can be emitted for everyone else
...
setTimeout(() => {
    ...
    setTimeout(() => {
        $('#position-13 .tutorialX').text('> tap <');
