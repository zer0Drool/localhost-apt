                    ...
                    if (transitionCounter === 0) { // user is now transitioning / only fires once
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                location.href = `http://192.168.4.1:8080/henry_pope?colour=${user.colour}`;
...
function heartClick() { // the user has created a heart
    ...
    socket.emit('newHeart', {colour: user.colour, userPosition: 'john'}); // sends heart to the server so it can be emitted for everyone else
...
function closeTutorial(e) {
    removeTutorialClick(1); // make sure the only id clickable is the specific wrap
}
...
var videoWrap = document.getElementById('john-video-wrap');
var videos = document.getElementsByClassName('john-video');
var videoOpen = false;
for (var i = 0; i < tickers.length; i++) {
    tickers[i].addEventListener('click', (e) => {
        if (videoOpen) {
            if (videoOpen === 1) {
            ...
            if (e.target.id === 'tick-one') {
                videos[0].style.display = 'block';
                videos[0].play();
                videoOpen = 1;
            } else if (e.target.id === 'tick-two') {
